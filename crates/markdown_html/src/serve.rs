...
async fn service(request: Request<body::Incoming>, input_dir: &Path, output_dir: &Path) -> Result<Response<Full<Bytes>>, Error> {
    match (request.method(), request.uri().path()) {
        ...
        (&Method::GET, path) => {
            let mut input_path = input_dir.join(Path::new(path).strip_prefix("/").unwrap());
            ...
            let mut output_path = output_dir.join(input_path.strip_prefix(input_dir).unwrap());
            output_path.set_extension("html");

            println!("{:?} {:?}", input_path, output_path);
            Ok(Response::new(Full::<Bytes>::from(markdown_to_html(&input_path, &output_path).unwrap())))
        }
        _ => {
